...
#ifdef NEWSHIP
    if (AttType == T_NEWSHIP) {
        if(ZielType == T_RAK || ZielType == T_LL || ZielType == T_SAT)
            return CSpeedKernel::GetInstance().RandomNumber(10000) >= 167;
        if(ZielType == T_KT || ZielType == T_LJ)
            return rand() % 100 >= 2;
        if(ZielType == T_SL || ZielType == T_IONEN || ZielType == T_GT || ZielType == T_SJ || ZielType == T_REC)
            return CSpeedKernel::GetInstance().RandomNumber(1000) >= 225;
        if(ZielType == T_SPIO)
            return rand() % 100 >= 1;
        if(ZielType == T_GAUSS || ZielType == T_KREUZER || ZielType == T_KOLO)
            return rand() % 100 >= 5;
        if(ZielType == T_SS || ZielType == T_BOMBER)
            return rand() % 10 >= 1;
        if(ZielType == T_PLASMA || ZielType == T_ZER)
            return rand() % 100 >= 20;
        if(ZielType == T_TS || ZielType == T_KS || ZielType == T_GS)
			return false;
    }
#endif
...
bool CSpeedKernel::CanShootAgain_V065(ITEM_TYPE AttType, ITEM_TYPE ZielType)
{
    ...
    if((ZielType == T_SPIO || ZielType == T_SAT) && AttType != T_SPIO && AttType < T_RAK)
		return rand() % 100 >= 20;		    // RF(5)
...
bool CSpeedKernel::InitSim()
{
	...
	for(i = 0; i < MAX_PLAYERS_PER_TEAM; i++)
	{
		for(int j = 0; j < T_RAK; j++)
		{
			m_NumShipsAtt[i*T_RAK+j].OwnerID = i;
			m_NumShipsAtt[i*T_RAK+j].Type = (ITEM_TYPE)j;
			m_NumShipsAtt[i*T_RAK+j].Num = 0;
		}
	}
	for(i = 0; i < MAX_PLAYERS_PER_TEAM; i++)
	{
