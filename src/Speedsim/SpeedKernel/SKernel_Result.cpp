...
bool CSpeedKernel::GetFleetAfterSim(SItem* Attacker, SItem* Defender, int FleetID)
{
  ...
  std::copy(items.begin() , items.end() , m_NumShipsDef.begin());
  ...
  if(Attacker && m_NumShipsAtt.size()) {
    ...
    for(i = 0; i < m_NumShipsAtt.size(); i++)
    {
      if(m_NumShipsAtt[i].OwnerID == FleetID){
        ...
        double  d    = Attacker[m_NumShipsAtt[i].Type].Num;
        __int64 ui   = __int64(d);
        if(d - ui >= 0.001){
          d = ui + 1;
        }
        else{
        ...
        Attacker[m_NumShipsAtt[i].Type].Num = d;
      }
    }
  }
  else if(Attacker) {
  ...
  if(Defender && m_NumShipsDef.size()) {
    ...
    for(i = 0; i < m_NumShipsDef.size(); i++)
    {
      if(m_NumShipsDef[i].OwnerID == FleetID){
        ...
        double  d    = Defender[m_NumShipsDef[i].Type].Num;
        __int64 ui   = __int64(d);
        if(d - ui >= 0.001){
          d = ui + 1;
        }
        ...
        Defender[m_NumShipsDef[i].Type].Num = d;
      }
    }
  }
  else if(Defender) {
...
bool CSpeedKernel::GetSetFleet(SItem* Attacker, SItem* Defender, int OwnerID)
{
  size_t i;
  DWORD Num = 0;
  if(Attacker && m_NumSetShipsAtt.size()) {
    ...
    {
      if(m_NumSetShipsAtt[i].OwnerID == OwnerID )
  ...
  if(Defender && m_NumSetShipsDef.size()) {
    ...
    {
      if(m_NumSetShipsDef[i].OwnerID == OwnerID )
...
bool CSpeedKernel::GenerateCR(const char* file)
{
  ...
  sprintf(c , _T("Õ½¶·±¨¸æ£º%s VS %s") , PlayerName[0].c_str() , PlayerName[1].c_str());
  out += _STR("<title>") + c +  _STR("</title></head>") + _STR("\n");
  ...
  for(j = 0; j < m_NumPlayersPerTeam[ATTER]; j++) {
    vector<SItem> vi;
    TCHAR title[128];
    _stprintf(title, _T("&nbsp;%s&nbsp;<br>") , PlayerName[0].c_str());
  ...
  if(!m_NumSetShipsAtt.size())
    out += _STR("<td>") + m_Attacker + _STR(" ½ø¹¥·½ <br><b>") + m_KBResult[8] + _STR("</b><br><br></td>") + _STR("\n");
  ...
  for(j = 0; j < m_NumPlayersPerTeam[DEFFER]; j++) {
    vector<SItem> vi;
    TCHAR title[128];
    _stprintf(title, _T("&nbsp;%s&nbsp;<br>") , PlayerName[1].c_str());
  ...
  if(!m_NumSetShipsDef.size())
    out += _STR("<td>") + m_Defender + _STR(" ·ÀÊØ·½ <br><b>") + m_KBResult[8] + _STR("</b><br><br></td>") + _STR("\n");
  ...
  for(int round = 1; round < m_Result.NumRounds; round++)
  {
    ...
    out += _T("<center>");
    TCHAR ch[64];
    sprintf(ch , _T("&nbsp;%s&nbsp;") , PlayerName[0].c_str());
    GenerateCRTable(out, m_NumShipsInKBAtt[round], ATTER, -1, ch);
    out += _T("</center>");
    out += _T("</td></tr></table>");
    out += _T("\n");
    out += _T("<table><tr><td>");
    out += _T("<center>");
    sprintf(ch , _T("&nbsp;%s&nbsp;") , PlayerName[1].c_str());
    GenerateCRTable(out, m_NumShipsInKBDef[round], DEFFER, -1, ch);
    out += _T("</center>");
    out += _T("</td></tr></table>");
    out += _T("</center>");
    out += _T("\n");
...
__int64 CSpeedKernel::GetFuel(PlaniPos& self_pos , PlaniPos& target_pos , const vector<SItem>& vFleet)
{
  __int64 result = 0;
  // fuel calculation
  float minSpeed = 99999999.0f;
  unsigned int uiTime = 0;
  // get minimum speed of all fleets
  float sp = GetFleetSpeed(0, vFleet);
  if(sp < minSpeed)
    minSpeed = sp;
  // compute separate fuel need for every fleet
  int dist = GetDistance(self_pos, target_pos);
  uiTime = ComputeFlyTime(self_pos, target_pos, 0, vFleet);
  __int64 gesverb = 0;
  // sum consumptions
  if(m_NewFuel) {
    // OGame >= v0.68a
    for(size_t i = 0; i < vFleet.size(); i++) {
      if(vFleet[i].OwnerID == 0) {
        int type = vFleet[i].Type;
        float basesp = BaseSpeed[type];
        int engine = Triebwerke[type];
        int iConsumpt = Verbrauch[type];
        if(type == T_KT && m_TechsTW[0][engine + 1] >= 5) {
          basesp *= 2;
          engine += 1;
          iConsumpt *= 2;
        }
        if(type == T_BOMBER && m_TechsTW[0][engine + 1] >= 8) {
          basesp = 5000;
          engine += 1;
        }
        if(type == T_SAT){
        ...
        basesp *= (1 + (float)m_TechsTW[0][engine] * (engine + 1) / 10.0f);
        float sp = 35000.f / (uiTime - 10) * sqrt(dist * 10 / basesp);
        gesverb += vFleet[i].Num * iConsumpt * (sp / 10.0f + 1) * (sp / 10.0f + 1);
      }
    }
    result = gesverb * dist / 35000.0f + 1;
  }
  else {
    // OGame < v0.68a
    for(size_t i = 0; i < vFleet.size(); i++) {
      if(vFleet[i].OwnerID == 0)
        gesverb += vFleet[i].Num * Verbrauch[vFleet[i].Type];        
    }
    result = gesverb * dist / 35000.0f * (m_Speed[0]/10.0f+1) * (m_Speed[0]/10.0f+1)+1;
  }
  return result;
}
