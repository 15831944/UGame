...
struct SPEEDKERNEL_API PlaniPos
{
  ...
  //! Constructor
  //! Sets all positions to 1
  PlaniPos() { Gala = 1; Sys = 1; Pos = 1; bMoon = false; bRuin = false;}

  //! Constructor to set positions immediately
  PlaniPos(int g, int s, int p) { Gala = g; Sys = s; Pos = p; bMoon = false; bRuin = false;}
  PlaniPos(int g, int s, int p , bool r , bool m) { Gala = g; Sys = s; Pos = p; bMoon = m; bRuin = r;}

  //! Constructor, where first param contains string with the position in format GG:SSS:PP
  bool operator==(const PlaniPos& other)
  {
    return Gala == other.Gala && Sys == other.Sys && Pos == other.Pos && bMoon ==other.bMoon && bRuin == other.bRuin;
  }

  bool OnlySameGSP(const PlaniPos& other)
  {
    return Gala == other.Gala && Sys == other.Sys && Pos == other.Pos && (bMoon != other.bMoon || bRuin != other.bRuin);
  }

  PlaniPos(const PlaniPos& other)
  {
    Gala = other.Gala ; Sys = other.Sys ; Pos = other.Pos ; bMoon =other.bMoon ; bRuin = other.bRuin;
  }

  PlaniPos& operator=(const PlaniPos& other)
  {
    if(this == &other)
    {
      return *this;
  ...
  char* ToCString(char* buf)
  {
    sprintf(buf, _T("%d:%d:%d"), Gala, Sys, Pos);  
    return buf;
  }
};
