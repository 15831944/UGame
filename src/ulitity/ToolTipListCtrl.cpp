   ...
   *pResult = 0;
   ...
   if( nFlags & m_wHitMask ){
      ...
      if( m_ToolTipMap.Lookup(strKey, strTipText ) ){
        ::SendMessage(pNMHDR->hwndFrom , TTM_SETMAXTIPWIDTH , 0 , (LPARAM)1000);
         ...
         //Deal with UNICODE
	      ...
	      if (pNMHDR->code == TTN_NEEDTEXTA)
		      lstrcpyn(pTTTA->lpszText, strTipText, 80);
	      else
		      _mbstowcsz(pTTTW->lpszText, strTipText, 80);
      #else
	      if (pNMHDR->code == TTN_NEEDTEXTA)
		      _wcstombsz(pTTTA->lpszText, strTipText, 80);
	      else
		      lstrcpyn(pTTTW->lpszText, strTipText, 80);
      #endif
