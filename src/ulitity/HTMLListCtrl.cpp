...
CHTMLListCtrl::CHTMLListCtrl()
{
  ...
  _tcscpy(lf.lfFaceName,"ËÎÌו");
  m_font.CreateFontIndirect(&lf);
  
  m_clrBkSelectedItem = RGB(60 , 80 , 150);
  m_bk_color          = RGB(55 , 77 , 112);
  m_list_color        = RGB(55 , 77 , 112);
...
int CHTMLListCtrl::InsertItem(CString sText, UINT uiImage, int nStyle, int nHeight , BOOL bTail)
{
  //Allocate memory
...
void CHTMLListCtrl::DeleteAllItems()
{
  ...
  pNMHDR->hdr.code = HTMLLIST_ITEMDELETEALL;
  ...
  for(int i = 0;i < m_listItems.GetCount();i++)
  {
    ...
    if(pItem->rcItem.PtInRect(point))
    {
      if(m_dwExtendedStyles & HTMLLIST_STYLE_CHECKBOX)
      {
        ...
        CRect rcBox(pt,CPoint(pt.x + 16,pt.y + 16));
...
BOOL CHTMLListCtrl::DeleteItem(int nPos)
{
  ...
  for(int i = 0;i < m_listItems.GetCount();i++)
  {
    ...
    {
      NM_HTMLLISTCTRL *pNMHDR = new NM_HTMLLISTCTRL;  
      pNMHDR->hdr.code = HTMLLIST_ITEMDELETE;
      pNMHDR->hdr.hwndFrom = GetSafeHwnd();
      pNMHDR->hdr.idFrom = m_nControlID;  
      pNMHDR->lItemData = 0;
      pNMHDR->nItemNo = nPos;
...
{	
  ...
  pNMHDR->hdr.idFrom = m_nControlID;  
  pNMHDR->lItemData = 0;
  pNMHDR->nItemNo = GetSelectedItem();
  pNMHDR->sItemText = "";
  pNMHDR->bChecked = 0;
  GetParent()->SendMessage(WM_NOTIFY,m_nControlID,(LPARAM)pNMHDR);  
  delete pNMHDR;
...
void CHTMLListCtrl::DrawItem(CDC *pDC, CRect rcItem, HTMLLIST_ITEM *pItem, BOOL bSelected)
{
  ...
  {
    pDC->FillSolidRect(&rcItem , m_bk_color);
  }
...
{
  SetCursor(AfxGetApp()->LoadCursor(IDC_CUR_LIST));
  return TRUE;
}

void CHTMLListCtrl::SetBKColor(COLORREF col)
{
  m_bk_color = col;
  if(m_hWnd != NULL)
    Invalidate();
...
void CHTMLListCtrl::SetSelectedColor(COLORREF col)
{
  m_clrBkSelectedItem = col;
  if(m_hWnd != NULL)
    Invalidate();
}

void CHTMLListCtrl::AdjustScrollBar()
{
  SCROLLINFO scrlinfo;
  scrlinfo.cbSize = sizeof(scrlinfo);
  scrlinfo.fMask = SIF_PAGE|SIF_RANGE;
...
void CHTMLListCtrl::SetListColor(COLORREF col)
{
  m_list_color = col;
  if(m_hWnd != NULL)
    Invalidate();
