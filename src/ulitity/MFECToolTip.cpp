...
BEGIN_MESSAGE_MAP(CMFECToolTip, CWnd)
...
void CMFECToolTip::OnKillFocus(CWnd* pNewWnd) 
{
}
...
CMFECToolTip::CalculateHeightAndWidth(CStringArray& straInfos)
{
	...
	m_nHeight = 8 + nLine * (m_nFontSize - 1);	//m_nFontSize is the font size
	m_nWidth =  6 + (int)(6 * nMaxLength);	//aproximately 6 pixel per char
}
...
void CMFECToolTip::CalculateInfoBoxRect(UINT nControlID, CRect* pInfoRect)
{
	...
	SetFontSize( 12 );
	int nBottom = oRect.bottom + m_nHeight;
	...
	else
	{
		...
		{
			SetFontSize( 10 );
			m_nHeight = 6 + m_nTotalLine * (m_nFontSize - 1);	//m_nFontSize is the font size
			m_nWidth =  4 + (int)(5 * m_maxCharInLine);	
		}
		// end
		pInfoRect->bottom = oRect.top;
...
void CMFECToolTip::DisplayInfo( ToolTipInfo* pToolTip )
{
	...
	oFont.CreateFont(m_nFontSize, 0, 0, 0, FW_REGULAR, 0, 0, 0, 0, 0, 0, 0, 0, "ËÎÌו");
